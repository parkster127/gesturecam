...
class NativeUI:
    ...
    def __init__(self, controller=None):
        ...
        self.on_mirror_toggle: Optional[Callable] = None
        self.on_virtual_camera_toggle: Optional[Callable] = None
    ...
    def setup(self):
        ...
        # Main window
        with dpg.window(label="GestureCam", tag="main_window", 
                       width=self.WINDOW_WIDTH, height=self.WINDOW_HEIGHT,
                       no_title_bar=False, no_resize=False):
            ...
            with dpg.group(horizontal=True):
                ...
                with dpg.group():
                    ...
                    with dpg.group(horizontal=True):
                        ...
                        with dpg.child_window(width=self.VIDEO_WIDTH + 16, 
                    ...
                    # === QUICK CONTROLS ===
                    with dpg.group(horizontal=True):
                        dpg.add_button(label="<-> Mirror", tag="mirror_btn",
                                      callback=self._toggle_mirror, width=100, height=36)
                        dpg.add_spacer(width=10)
                        dpg.add_button(label="[O] Overlay", tag="overlay_btn",
                                      callback=self._toggle_overlay, width=100, height=36)
                        dpg.add_spacer(width=10)
                        dpg.add_button(label="[P] Pause", tag="pause_btn",
                                      callback=self._toggle_pause, width=100, height=36)
                    ...
                    with dpg.group(horizontal=True):
                        ...
                        dpg.add_button(label="Settings", callback=self._on_settings_click, 
                                      width=90, height=36)
                        dpg.add_spacer(width=10)
                        dpg.add_button(label="Start Virtual Camera", 
                                      callback=self._on_virtual_camera_click,
                                      tag="virtual_camera_btn", width=150, height=36)
    ...
    def update_state(self, gesture: str = None, zoom: float = None, 
                     camera_name: str = None, latency: float = None):
    ...
    def _on_settings_click(self):
        ...
        if not dpg.does_item_exist("settings_popup"):
            with dpg.window(
                ...
                dpg.add_slider_float(
                    label="Hand", 
                    default_value=0.5, min_value=0.1, max_value=1.0,
                    tag="sensitivity_hand", width=250
                )
                dpg.add_slider_float(
                    label="Face", 
                    default_value=0.5, min_value=0.1, max_value=1.0,
                    tag="sensitivity_face", width=250
                )
...
def run_demo():
    ...
    for model_path, url, name in [
        (hand_model_path, "https://storage.googleapis.com/mediapipe-models/hand_landmarker/hand_landmarker/float16/1/hand_landmarker.task", "hand"),
        (face_model_path, "https://storage.googleapis.com/mediapipe-models/face_landmarker/face_landmarker/float16/1/face_landmarker.task", "face"),
    ]:
    ...
    def video_thread():
                ...
                try:
                    ...
                    if results.hand_landmarks:
                        ...
                        for i, landmarks in enumerate(results.hand_landmarks):
                            ...
                            center = (int(sum(lm.x for lm in landmarks)/21*w),
                        ...
                        if len(centers) == 2:
                            dist = math.sqrt((centers[0][0]-centers[1][0])**2 + 
        ...
        cap.release()
        if hand_detector: hand_detector.close()
